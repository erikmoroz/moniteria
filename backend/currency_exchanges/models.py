...
class CurrencyExchange(models.Model):
    ...
    from_currency = models.ForeignKey(
        'workspaces.Currency', on_delete=models.PROTECT, related_name='exchanges_from'
    )
    from_amount = models.DecimalField(max_digits=15, decimal_places=2)
    to_currency = models.ForeignKey(
        'workspaces.Currency', on_delete=models.PROTECT, related_name='exchanges_to'
    )
    ...
    def __str__(self):
        return f'{self.date} - {self.from_amount} {self.from_currency.symbol} → {self.to_amount} {self.to_currency.symbol}'
